...
$dsn = $repo->getDosenProfile($userId);

if (!$dsn) {
    die("Data dosen tidak ditemukan.");
}

$_SESSION['nama_lengkap'] = $dsn['nama'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($repo->updateBiodata($dsn['id'], $dataUpdate)) {
        ...
        if (isset($_FILES['foto']) && $_FILES['foto']['error'] === UPLOAD_ERR_OK) {
            ...
            if (in_array($ext, $allowed)) {
                ...
                $newFileName = 'dsn_' . $dsn['nidn'] . '_' . time() . '.' . $ext;
                $targetPath = $uploadDir . $newFileName;
                if (move_uploaded_file($tmp_name, $targetPath)) {
                    $repo->updateFoto($dsn['id'], 'uploads/foto/' . $newFileName);
                }
            }
        }
...
$foto = $dsn['foto'] ?? 'https://ui-avatars.com/api/?name=' . urlencode($dsn['nama']) . '&background=random';
$title = "Profil Dosen - SAQUNA";
            ...
            <section class="lg:col-span-1">
                <div class="bg-white rounded-3xl p-stack-md shadow-sm border border-outline-variant h-full relative overflow-hidden group">
                    ...
                    <div class="text-center mb-6 relative z-10">
                        ...
                        <h2 class="font-title-lg text-title-lg text-on-surface font-bold"><?= htmlspecialchars($dsn['nama']) ?></h2>
                        <p class="font-body-md text-primary mt-1 font-mono tracking-wider"><?= htmlspecialchars($dsn['nidn']) ?></p>
                        <span class="inline-block mt-3 px-3 py-1 bg-secondary-container text-on-secondary-container rounded-full text-xs font-semibold">Dosen Aktif</span>
                    </div>
                    ...
                    <div class="space-y-4 relative z-10">
                        ...
                        <div class="bg-surface p-3 rounded-xl border border-outline-variant/50">
                            ...
                            <p class="font-body-md text-on-surface font-medium"><?= htmlspecialchars($dsn['program_studi'] ?? '-') ?></p>
                        </div>
                        
                        <div class="bg-surface p-3 rounded-xl border border-outline-variant/50">
                            <p class="text-xs text-on-surface-variant mb-1 uppercase tracking-wide font-semibold">Fakultas</p>
                            <p class="font-body-md text-on-surface font-medium"><?= htmlspecialchars($dsn['fakultas'] ?? 'Belum diset') ?></p>
                        </div>
                        
                        <div class="mt-4 p-3 bg-secondary/10 rounded-lg text-xs text-secondary-container-on flex gap-2 items-start">
                            <span class="material-symbols-outlined text-[16px]">info</span>
                            <p>Data fungsional di atas hanya dapat diubah oleh Operator Administrasi Universitas.</p>
                        </div>
                    </div>
                </div>
            </section>
                ...
                <div class="bg-white rounded-3xl p-stack-md shadow-sm border border-outline-variant h-full">
                    <form method="POST" action="" enctype="multipart/form-data" class="h-full flex flex-col">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 flex-1">
                            ...
                            <!-- Nama Lengkap -->
                            <div class="md:col-span-2">
                                <label class="block text-sm font-semibold text-on-surface-variant mb-2">Nama Lengkap & Gelar</label>
                                <input type="text" name="nama" value="<?= htmlspecialchars($dsn['nama']) ?>" class="w-full bg-surface border border-outline-variant rounded-xl px-4 py-3 focus:ring-2 focus:ring-primary focus:border-primary outline-none transition-all" required>
                            </div>

                            <!-- Tempat, Tanggal Lahir -->
                            <div class="md:col-span-2">
                                <label class="block text-sm font-semibold text-on-surface-variant mb-2">Tempat, Tanggal Lahir</label>
                                <input type="text" name="tempat_tanggal_lahir" value="<?= htmlspecialchars($dsn['tempat_tanggal_lahir'] ?? '') ?>" placeholder="Cth: Malang, 17 Agustus 1980" class="w-full bg-surface border border-outline-variant rounded-xl px-4 py-3 focus:ring-2 focus:ring-primary focus:border-primary outline-none transition-all">
                            </div>
                            ...
                            <div>
                                ...
                                <select name="jenis_kelamin" class="w-full bg-surface border border-outline-variant rounded-xl px-4 py-3 focus:ring-2 focus:ring-primary focus:border-primary outline-none transition-all">
                                    <option value="" <?= empty($dsn['jenis_kelamin']) ? 'selected' : '' ?>>-- Pilih --</option>
                            ...
                            <div>
                                ...
                                <input type="text" name="no_hp" value="<?= htmlspecialchars($dsn['no_hp'] ?? '') ?>" placeholder="Cth: 081234567890" class="w-full bg-surface border border-outline-variant rounded-xl px-4 py-3 focus:ring-2 focus:ring-primary focus:border-primary outline-none transition-all">
                            </div>

                            <!-- Email -->
                            <div class="md:col-span-2">
                                <label class="block text-sm font-semibold text-on-surface-variant mb-2">Alamat Email Pribadi</label>
                                <input type="email" name="email" value="<?= htmlspecialchars($dsn['email'] ?? '') ?>" placeholder="Cth: nama@gmail.com" class="w-full bg-surface border border-outline-variant rounded-xl px-4 py-3 focus:ring-2 focus:ring-primary focus:border-primary outline-none transition-all">
                            </div>

                            <!-- Alamat Asal -->
                            <div class="md:col-span-2">
                                <label class="block text-sm font-semibold text-on-surface-variant mb-2">Alamat Asal (Sesuai KTP)</label>
                                <textarea name="alamat_asal" rows="2" class="w-full bg-surface border border-outline-variant rounded-xl px-4 py-3 focus:ring-2 focus:ring-primary focus:border-primary outline-none transition-all"><?= htmlspecialchars($dsn['alamat_asal'] ?? '') ?></textarea>
                            </div>

                            <!-- Domisili -->
                            <div class="md:col-span-2">
                                <label class="block text-sm font-semibold text-on-surface-variant mb-2">Alamat Domisili saat ini</label>
                                <textarea name="domisili" rows="2" class="w-full bg-surface border border-outline-variant rounded-xl px-4 py-3 focus:ring-2 focus:ring-primary focus:border-primary outline-none transition-all"><?= htmlspecialchars($dsn['domisili'] ?? '') ?></textarea>
                            </div>
