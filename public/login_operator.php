...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!Auth::validateCsrf($_POST['csrf_token'] ?? '')) {
        ...
        if (empty($username) || empty($password)) {
            ...
            $result = Auth::login($username, $password, 'operator');
            if ($result['success']) {
                header("Location: dashboard.php");
                exit;
            } else {
...
<head>
    ...
    <title>SAQUNA | Operator Portal Login</title>
    ...
    <style>
        ...
        @keyframes pulse-ring {
            0% {
                ...
                box-shadow: 0 0 0 0 rgba(25, 107, 80, 0.4);
            }

            70% {
                transform: scale(1);
                box-shadow: 0 0 0 10px rgba(25, 107, 80, 0);
            }

            100% {
                transform: scale(0.98);
                box-shadow: 0 0 0 0 rgba(25, 107, 80, 0);
            }
        }
            ...
            <div class="flex items-center gap-stack-sm text-on-surface-variant font-body-md">
                <div class="group relative flex items-center justify-center">
                    ...
                    <div class="absolute top-full right-0 mt-2 w-64 bg-surface-container-high text-on-surface p-3 rounded-lg shadow-lg text-sm border border-outline-variant/30 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50 font-normal">
                        Halaman login khusus Operator/Staff. Silakan masukkan kredensial Anda untuk mengakses portal akademik SAQUNA.
                    </div>
                </div>
            ...
            <div class="hidden lg:flex flex-col justify-center items-start space-y-stack-md">
                <div class="relative w-full aspect-square max-w-[400px]">
                    ...
                    <div class="absolute inset-0 rounded-full glass-panel border-mint-glow animate-float flex items-center justify-center overflow-hidden">
                        <img alt="Futuristic geometry" class="w-full h-full opacity-40 scale-150 rotate-12" src="https://lh3.googleusercontent.com/aida-public/AB6AXuDHdKx8JEashZFZ_m2D_dYYYlIWX9XYm7WzPHIPMH35Ei7RXHw_AIPO2TUVh-DZnIdbR7UEvUfVK2utQvuQaHKuxwPHQgg33DELQ6Vbam5ETV859Uy0huRfLoApThDV6hPNZ1iMLAmkTsGc9rXKqHqfAOGjkX4Rq3-TOIAq_J7-nSsBRrCfgtj7P18wiESJdVZF7mImJPhaoVENr0dvv675i8ZogAp9J-ZZ7DhBtV39GRApPLnbISccb68UWu54MDpYPrrOZBI3KyM" />
                ...
                <div class="space-y-stack-sm">
                    <h1 class="font-display-lg text-display-lg text-primary tracking-tight">Central Administration Hub.</h1>
                    <p class="font-body-lg text-body-lg text-on-surface-variant max-w-[380px]">Manage institutional data and student affairs securely and efficiently with friction-less elegance.</p>
                </div>
            </div>
            <!-- Right Side: Login Card -->
            <div class="flex justify-center">
                <div class="glass-panel w-full max-w-[480px] p-10 rounded-[2rem] mint-glow-shadow flex flex-col items-center relative">
                    <div class="w-full text-center mb-stack-lg">
                        <h2 class="font-headline-lg text-headline-lg text-on-surface mb-2">Login Operator</h2>
                        <p class="font-body-md text-body-md text-on-surface-variant">Portal Super Admin & Staf Akademik.</p>
                    </div>
                    ...
                    <form method="POST" class="w-full space-y-stack-md">
                        ...
                        <!-- Input 1 -->
                        <div class="space-y-2">
                            <label class="font-label-md text-label-md text-on-surface-variant ml-1">Username Operator</label>
                            <div class="relative group">
                                <span class="material-symbols-outlined absolute left-4 top-1/2 -translate-y-1/2 text-on-surface-variant group-focus-within:text-primary transition-colors">admin_panel_settings</span>
                                <input name="username" class="w-full bg-surface-container-lowest border border-outline-variant rounded-xl py-4 pl-12 pr-4 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all text-on-surface placeholder:text-outline-variant" placeholder="Masukkan Username" type="text" required />
                            </div>
                        </div>
                        <!-- Input 2 -->
                        <div class="space-y-2">
