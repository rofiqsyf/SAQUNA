    ...
    if (!Auth::validateCsrf($_POST['csrf_token'] ?? '')) {
        ...
        if ($penerimaId > 0 && $subjek !== '' && $pesan !== '') {
            if ($repo->kirimPesan($userId, $penerimaId, $subjek, $pesan)) {
                $success = "Pesan balasan berhasil dikirim!";
            } else {
...
$daftarKontak = $repo->getSemuaPengguna();

$title = "Helpdesk Operator";
$current_page = "operator_chat.php";
include 'components/header.php';
?>

<div class="mb-4 d-flex justify-content-between align-items-center">
    <div>
        <h1>Pusat Bantuan (Helpdesk)</h1>
        <p class="text-on-surface-variant opacity-80">Tangani keluhan atau pertanyaan administratif dari Dosen dan Mahasiswa.</p>
    </div>
    <button class="bg-primary hover:bg-on-primary-fixed-variant text-on-primary font-label-md px-6 py-3 rounded-xl shadow-lg shadow-primary-container/20 transition-all active:scale-95 inline-block text-center" onclick="document.getElementById('formPesanBaru').style.display = 'block'">+ Tulis Pesan Baru</button>
</div>
...
<div id="formPesanBaru" class="glass-panel rounded-3xl p-stack-md mb-stack-md shadow-sm border border-white/40" style="display: none; border-top: 4px solid var(--primary);">
    <div class="glass-panel rounded-3xl p-stack-md mb-stack-md shadow-sm border border-white/40">
        <h3 class="mt-0">Kirim Pesan (Broadcast / Personal)</h3>
        <form method="POST">
            <?= Auth::csrfField() ?>
            <div class="mb-stack-sm">
                <label class="block font-label-md text-label-md text-on-surface-variant mb-2">Kirim Ke (Dosen / Mahasiswa)</label>
            ...
            <div class="mb-stack-sm">
                ...
                <input type="text" name="subjek" class="w-full bg-white/60 border border-outline-variant/50 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-primary-container focus:border-primary transition-all font-body-md placeholder:text-outline-variant" placeholder="Contoh: Jawaban Keluhan KRS" required>
            </div>
            <div class="mb-stack-sm">
                <label class="block font-label-md text-label-md text-on-surface-variant mb-2">Isi Pesan</label>
                <textarea name="pesan" class="w-full bg-white/60 border border-outline-variant/50 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-primary-container focus:border-primary transition-all font-body-md placeholder:text-outline-variant" rows="5" required></textarea>
            </div>
...
<?php if (empty($pesanMasuk)): ?>
    <div class="bg-tertiary-container text-on-tertiary-container p-stack-sm rounded-xl mb-stack-md font-body-md flex items-center gap-2">Belum ada pesan atau tiket keluhan yang masuk.</div>
<?php else: ?>
    <div class="row" style="display: grid; grid-template-columns: 1fr; gap: 1rem;">
        <?php foreach ($pesanMasuk as $p): ?>
        <div class="glass-panel rounded-3xl p-stack-md mb-stack-md shadow-sm border border-white/40" style="border-left: 4px solid var(--warning);">
            <div class="glass-panel rounded-3xl p-stack-md mb-stack-md shadow-sm border border-white/40">
                ...
                <div class="mb-3">
                    <span class="badge badge-<?= $p['pengirim_role'] === 'dosen' ? 'success' : 'primary' ?>">
                        Pengirim: <?= strtoupper(htmlspecialchars($p['pengirim_role'])) ?>
                    </span>
                    <strong style="margin-left: 8px;"><?= htmlspecialchars($p['pengirim_nama']) ?></strong>
                </div>
                <p style="background: var(--bg-light); padding: 1rem; border-radius: 6px; margin: 0;">
