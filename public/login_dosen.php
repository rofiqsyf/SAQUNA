...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!Auth::validateCsrf($_POST['csrf_token'] ?? '')) {
        ...
        if (empty($username) || empty($password)) {
            ...
            $result = Auth::login($username, $password, 'dosen');
            if ($result['success']) {
                header("Location: dosen_dashboard.php");
                exit;
            } else {
...
<title>SAQUNA | Dosen Portal Login</title>
...
<div class="group relative flex items-center justify-center">
    ...
    <div class="absolute top-full right-0 mt-2 w-64 bg-surface-container-high text-on-surface p-3 rounded-lg shadow-lg text-sm border border-outline-variant/30 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50 font-normal">
        Halaman login khusus Dosen. Silakan masukkan kredensial Anda untuk mengakses portal akademik SAQUNA.
    </div>
</div>
...
<h1 class="font-display-lg text-display-lg text-primary tracking-tight">Transforming Academic Data into Clarity.</h1>
<p class="font-body-lg text-body-lg text-on-surface-variant max-w-[380px]">A modern ecosystem designed for lecturers to manage scholarly progress with frictionless elegance.</p>
</div>
</div>
...
<h2 class="font-headline-lg text-headline-lg text-on-surface mb-2">Selamat Datang Dosen</h2>
<p class="font-body-md text-body-md text-on-surface-variant">Portal Khusus Tenaga Pengajar.</p>
</div>
...
<!-- Input 1 -->
<div class="space-y-2">
<label class="font-label-md text-label-md text-on-surface-variant ml-1">Username atau NIDN</label>
...
<a class="font-label-md text-label-md text-primary hover:underline" href="lupa_sandi.php?role=dosen">Lupa Password?</a>
</div>
<div class="relative group">
