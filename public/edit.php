...
Auth::requireLogin();
// Operator BISA mengakses edit, jadi tidak perlu requireAdmin()
...
$mkIds = $repo->getDosenMataKuliahIds($id);

$errors = [];
$old = array_merge($dosen, ['matakuliah' => $mkIds]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!Auth::validateCsrf($_POST['csrf_token'] ?? '')) {
        ...
        $fotoName = $validator->validateImageUpload('foto', false);
        ...
        if (!$validator->hasErrors()) {
            $data = [
                ...
                'foto' => $fotoName // Akan menjadi null jika tidak ada file baru
            ];
            ...
            if ($fotoName) {
                ...
                if (!$uploadSuccess) {
                    $errors['foto'] = 'Gagal menyimpan file foto baru.';
                } else {
                    // Update data dengan path yang tersinkronisasi
                    $data['foto'] = 'uploads/foto/' . $fotoName;
                }
            }

            if (empty($errors)) {
                $success = $repo->update($id, $data, (array)$matakuliahIds, $_SESSION['user_id']);
                if ($success) {
                    // Hapus foto lama jika update berhasil dan ada file baru yang diunggah
                    if ($fotoName && $uploadSuccess && $dosen['foto']) {
                        $oldPath = __DIR__ . '/' . $dosen['foto']; // Karena di DB berformat 'uploads/foto/...'
                    ...
                    $errors['db'] = 'Gagal memperbarui ke database. NIDN/Email mungkin sudah terdaftar pada dosen lain.';
                }
            }
        } else {
    ...
    <div class="glass-panel rounded-3xl p-stack-md mb-stack-md shadow-sm border border-white/40">
        <form method="POST" action="" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-stack-md">
                ...
                <div class="mb-stack-sm">
                    <label class="block font-label-md text-label-md text-on-surface-variant mb-2" for="foto">Ubah Foto Profil (Kosongkan jika tidak ingin mengubah)</label>
                    ...
                    <?php if ($dosen['foto']): ?>
                        <div class="mt-2 text-muted" style="font-size: 0.85rem;">Foto saat ini: <?= htmlspecialchars($dosen['foto']) ?></div>
                    <?php endif; ?>
            ...
            <div class="mt-4">
                <button type="submit" class="bg-primary hover:bg-on-primary-fixed-variant text-on-primary font-label-md px-6 py-3 rounded-xl shadow-lg shadow-primary-container/20 transition-all active:scale-95 inline-block text-center">Perbarui Data</button>
            </div>
        </form>
    </div>
</div>
